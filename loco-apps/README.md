...
`POST /login` is global (`{ "username" }`, optional `"password"`; no site headers required). `POST /logout`, `GET /me` (authenticated self-read), `POST /users` (self-service signup, password required), users list/update/delete (org owner of at least one org), API keys. Persistence is `auth/{accounts,identities,sessions,api_keys}/` (gitignored).
