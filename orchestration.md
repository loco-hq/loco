...
3. Do **not** write application code. Pick a shippable issue (see `CONTRIBUTING.md`). Spawn the **other** vendor as implementer via herdr (recipes below).
...
| Merge | Human (Ben) in phase 1. `main` requires a PR + 1 approving review; stale reviews dismissed; admins included |
| Active work | First real cycle using bot identities |
| Next | First product issue. Spawn Claude as implementer; Grok reviews. |
...
**Invariant:** implementer vendor ≠ reviewer vendor. The orchestrator may be the same vendor as the reviewer (the default shape) or a third live pane. The orchestrator is never the implementer for that issue.

OpenAI, if added later, is a third vendor with a third GitHub App. Same rules: reviewer is not the implementer.

## Cycle

One issue at a time is the default. Parallel issues are allowed when they do not share files.
...
3. **Spawn implementer.** Herdr worktree + `agent start` of the **other** vendor (see [Herdr recipes](#herdr-recipes)). Prompt names the issue number, the GitHub identity they must use, and that they open a PR rather than pushing `main`.
4. **Wait.** `herdr agent wait` until idle/done/blocked. Blocked (approval UI, missing token) is the orchestrator’s problem, not the reviewer’s.
5. **Spawn reviewer.** Other vendor (if the implementer was the default “other”, the reviewer is this orchestrator’s vendor — either this pane, or a fresh pane so the orchestrator stays a process role). Reviewer reads the PR via `gh`, posts a **GitHub** review, not a herdr sidebar comment as the record of decision.
6. **Address or escalate.**
...
7. **Human merge.** Ben merges when the review is an approval, CI is green (when we have it), and he agrees. Orchestrator does not merge.
8. **Close the loop.** Orchestrator updates `HANDOFF.md` if product state changed, this file if process changed, and the issue (close via the PR). Hand off the term when done sitting.
...
Reviewer can sit on the main checkout; they need the PR, not the worktree:
...
Read the diff with gh. Post a GitHub review: approve, comment, or request changes.
...
Approve when: the issue’s acceptance is met, you would be willing to have Ben merge it, and you are not the implementer.
...
Do not turn this file into a changelog of every merged PR.
