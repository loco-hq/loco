...
9. **Close the loop.**
   ...
   - **After merge:** close that issue’s implementer workspace (`herdr workspace close <id>`), close its reviewer pane (`herdr pane close <id>`). `workspace close` does not always delete the git worktree — if the checkout remains, `git worktree remove --force <path>` (the herdr workspace id is already gone). Do not close panes you did not create. Do not tear down a parent that still has an open child stacked on it.
